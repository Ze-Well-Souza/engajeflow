...
// Only import the functions being tested, avoid default import triggering EnvironmentManager instantiation
import { getEnvVariable, setEnvVariable, removeEnvVariable, validateEnvVariables } from '../../utils/environment';

// Set essential environment variables globally for the test suite
// This ensures they are set before any module imports might trigger validation
process.env.NODE_ENV = 'test';
process.env.TECHCARE_BASE_URL = 'http://test.com';
...
describe('Environment Utilities', () => {
  ...
  beforeEach(() => {
    // Restore essential vars potentially modified by tests
    process.env.NODE_ENV = 'test';
    process.env.TECHCARE_BASE_URL = 'http://test.com';
  ...
  describe('validateEnvVariables', () => {
    ...
    it('deve retornar valid=false e lista de variáveis faltantes', () => {
      ...
      localStorageMock.removeItem('env.VAR2');
      delete process.env.VAR3;
      localStorageMock.removeItem('env.VAR3');
      ...
      // Ensure the check is correct - getEnvVariable returns '' if not found
      // validateEnvVariables considers '' as missing.
