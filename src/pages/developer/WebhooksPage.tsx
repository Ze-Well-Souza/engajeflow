...
const WebhooksPage: React.FC = () => {
  ...
  const handleToggleWebhook = (id: string, currentStatus: boolean) => {
    ...
    alert(currentStatus ? "Webhook desativado" : "Webhook ativado");
  };
  ...
  const handleAddWebhook = () => {
    if (!newWebhookUrl || selectedEvents.length === 0) {
      alert("Por favor, insira uma URL e selecione pelo menos um evento.");
      return;
    }
  ...
  const handleTestWebhook = (id: string) => {
    alert("Um evento de teste foi enviado para este webhook.");
  };

  const copySecret = () => {
    navigator.clipboard.writeText("whsec_bF9nW8vE1hY3xZ2qP4jL5tK7mD0cR6");
    alert("O secret foi copiado para a área de transferência");
  };
